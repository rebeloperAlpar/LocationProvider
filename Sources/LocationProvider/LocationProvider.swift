...
public class LocationProvider: NSObject, ObservableObject {
    ...
    /// The authorisation status for CoreLocation.
    ...
    /// The LocationProvider intialiser.
    ///
    ...
    /**
     ...
     The default behaviour is to present an alert that suggests going to the settings page.
     */
    ...
    public func start(status: CLAuthorizationStatus = defaultCLAuthorizationStatus) throws -> Void {
        ...
        if let status = self.authorizationStatus {
            ...
            /// no authorisation set by delegate yet
            #if DEBUG
            ...
            /// However, this would prevent start() from running directly after the LocationProvider is initialised.
            /// This is because the delegate method `didChangeAuthorization`,
            /// setting `authorizationStatus` runs only after a brief delay after initialisation.
            //throw LocationProviderError.noAuthorization
        }
...
/// Error which is thrown for lacking localisation authorisation.
